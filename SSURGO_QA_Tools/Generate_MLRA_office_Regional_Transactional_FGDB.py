# Create_MLRA_office_Regional_Transactional_FGDB
#
# 1/16/2014
#
...
#
#
#Modified to create MLRA Office RSTD file geodatabases
#11/5/2014 A. Stephens
#Need to add ~ Enable Editor Tracking, Add Original MUSYM and FeatSYM fields to soils and sf feature classes, add Domain Yes or No to Project Record
#also add MUNAME field and National Mapunit symbol field(3/19/2015)
...
def createFGDB(regionChoice,outputFolder):
    ...
    try:
        ...
        if datetime.datetime.now().strftime("%m") > 9 and datetime.datetime.now().strftime("%m") < 14:
...
# Input is an integer.  Integer with or without thousands separator is returned.
...
# i.e. alias name for MUPOLYGON = Region 10 - Mapunit Polygon
    ...
    try:
        ...
        aliasUpdate = 0
        #regionNumber = str([int(s) for s in regionChoice() if s.isdigit()][0])
        regionNumber = "MLRA"

        if arcpy.Exists(os.path.join(fdPath,'FEATLINE')):
            arcpy.AlterAliasName(os.path.join(fdPath,'FEATLINE'), "RTSD " + regionNumber + " - Special Feature Lines")  #FEATLINE
            aliasUpdate += 1

        if arcpy.Exists(os.path.join(fdPath,'FEATPOINT')):
            arcpy.AlterAliasName(os.path.join(fdPath,'FEATPOINT'), "RTSD " + regionNumber + " - Special Feature Points")  #FEATPOINT
            aliasUpdate += 1

        if arcpy.Exists(os.path.join(fdPath,'MUPOLYGON')):
            arcpy.AlterAliasName(os.path.join(fdPath,'MUPOLYGON'), "RTSD " + regionNumber + " - Mapunit Polygon")  #MUPOLYGON
            aliasUpdate += 1

        if arcpy.Exists(os.path.join(fdPath,'SAPOLYGON')):
            arcpy.AlterAliasName(os.path.join(fdPath,'SAPOLYGON'), "RTSD " + regionNumber + " - Survey Area Polygon")  #SAPOLYGON
            aliasUpdate += 1

        if arcpy.Exists(os.path.join(fdPath,'MULINE')):
            arcpy.AlterAliasName(os.path.join(fdPath,'MULINE'), "RTSD " + regionNumber + " - Mapunit Line")  #MULINE
            aliasUpdate += 1

        if arcpy.Exists(os.path.join(fdPath,'MUPOINT')):
            arcpy.AlterAliasName(os.path.join(fdPath,'MUPOINT'), "RTSD " + regionNumber + " - Mapunit Point")  #MUPOINT
            aliasUpdate += 1

        if arcpy.Exists(os.path.join(FGDBpath + os.sep + 'ProjectRecord' + os.sep + 'Project_Record')):
            arcpy.AlterAliasName(os.path.join(FGDBpath + os.sep + 'ProjectRecord' + os.sep + 'Project_Record'), "RTSD " + regionNumber + " - Project Record")  #Project_Record
            aliasUpdate += 1
